                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Gizli arama başlattınız!");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Zaten bir aramadasınız!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Bu kişi meşgul!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Bu kişi müsait değil!");
            }
        } else {
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Hmm, bunu yapmam gerekmezdi...'
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefon", "Kendinize WhatsApp mesajı gönderemezsiniz..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, bunu yapmam gerekmezdi...'
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, bunu yapmam gerekmezdi...'
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Kişi Düzenle", "Tüm alanları doldurun!");
    }
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        if (SelectedSuggestion !== null) {
            ...
            $(".amount-of-suggested-contacts").html(amount + " kişi");
        }
    } else {
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Bu kişi aramada!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Bu kişi müsait değil!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Kendi numaranızı arayamazsınız!");
        }
    });
}
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = CallData.name+" sizi arıyor"
                if (AnonymousCall) {
                    Label = "Gizli numaradan aranıyorsunuz"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            $(".phone-currentcall-title").html("Aramada ("+cData.CallTime+")");
        }
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " kişi");
