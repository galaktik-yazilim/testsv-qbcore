...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-lastsync").html("Son senkronizasyon "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Hiç e-postanız yok..</p>');
        }
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "İlan", "Boş ilan paylaşamazsınız!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (clean == '') { clean = 'Ben aptal bir kazım :/' }
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Henüz ilan yok!</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://qb-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "İlan", "İlan silindi", "#ff8f1a", 2000);
        },400)
    });
})
